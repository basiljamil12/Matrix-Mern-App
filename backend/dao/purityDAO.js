...
export default class PurityDAO {
    ...
    static async addPurity(name, amount, beanSizeScore, beanColorScore, beanConsistencyScore, beanFreshnessScore, beanStiffIndexScore, beanRipeIndexScore, emp_id){
        try {
            const totalPurity = ((Number(beanSizeScore) + Number(beanColorScore) + Number(beanConsistencyScore) + Number(beanFreshnessScore) + Number(beanStiffIndexScore) + Number(beanStiffIndexScore) + Number(beanRipeIndexScore))/6)
            let actionStatus 
