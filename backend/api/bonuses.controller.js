...
export default class BonusesController {
    static async apiGetBonusesByID(req, res, next) {
        ...
        const { bonusList, totalNumBonuses } = await BonusesDAO.getBonusesByID ({filters,})
    ...
    static async apiPostBonus(req, res, next) {
        try {
            ...
            const bonusResponse = await BonusesDAO.addBonuses(
                name,
                amount,
                emp_id,
            )
    ...
    static async apiDeleteBonus(req, res, next) {
        try {
            ...
            const bonusResponse = await BonusesDAO.deleteBonuses(
                id,
            )
            res.json({ Status: "success" })
        } catch (e) {
